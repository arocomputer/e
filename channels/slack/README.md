...
Every release publishes the bot as `ghcr.io/intuitums/ulo-slack` (`:latest` on
...
docker run --rm -it --user "$(id -u):$(id -g)" \
  ...
  ghcr.io/intuitums/ulo-slack:latest trust
docker run --rm -it --user "$(id -u):$(id -g)" \
  -v ulo-slack-home:/home/ulo --entrypoint ulo ghcr.io/intuitums/ulo-slack:latest   # then /login

docker run -d --restart unless-stopped --name ulo-slack \
  --user "$(id -u):$(id -g)" --env-file .env -e ULO_CWD=/work \
  -v ulo-slack-home:/home/ulo -v "$PWD:/work" ghcr.io/intuitums/ulo-slack:latest
```
