...
Reference programs that put ulo in a team's tools by spawning `ulo rpc` and
...
compiled into ulo, and nothing here is required to run it. The pattern they
share is described in `docs/guides/usage/channels.md`.

```
slack/     a Slack bot: one process and session per thread (TypeScript, Bolt)
github/    a GitHub Actions workflow answering `/ulo` on issues and PRs
```
...
The Slack bot is also published as `@arocomputer/ulo-slack`, so it runs on a
machine that has no checkout of this repository (`npx @arocomputer/ulo-slack`).
