...
#[test]
fn diff_chip_replacement_preserves_the_draft_and_caret() {
    let mut editor = Editor::new();
    ...
    assert!(rendered.contains(
        &theme::load_bundled(false)
            .unwrap()
            .fg("diffSelectionText", "⧉ 16 lines from diff")
    ));
}

#[test]
fn diff_chip_backspace_selects_then_deletes_and_motion_cancels_selection() {
    let mut editor = Editor::new();
