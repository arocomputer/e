...
/// `e install`: make every listed package current, one line per package.
/// Fails when any one did.
async fn install_all() -> i32 {
    let results = e::core::resources::packages::install_all().await;
    if results.is_empty() {
        println!("{NO_PACKAGES}");
        return 0;
    }
...
/// `e packages`: each listed package and what it loads.
...
/// `e packages init <dir>`: scaffold a package and name the files written.
fn init_package(dir: &std::path::Path) -> i32 {
    match e::core::resources::packages::init(dir) {
        Ok(written) => {
...
fn describe_counts(counts: &[usize; 4]) -> String {
    let parts: Vec<String> = e::core::resources::packages::KINDS
        .iter()
        .zip(counts)
        .filter(|(_, n)| **n > 0)
        .map(|(kind, n)| {
...
async fn main() -> std::io::Result<()> {
    ...
    let args = match run_startup_hooks(&host, args, startup_json_requested).await {
        e::core::extensions::StartupAction::Continue(next) => next,
        e::core::extensions::StartupAction::Relaunch { argv, request } => {
            host.shutdown().await;
...
/// `e --version`: the release identity, as JSON with `--json`.
...
async fn start_host(
    ...
    requests: &tokio::sync::mpsc::Sender<e::core::extensions::HostRequest>,
) -> std::sync::Arc<e::core::extensions::ExtensionHost> {
...
async fn run_extension_free(host: &e::core::extensions::ExtensionHost, options: &Options) -> bool {
...
/// `e doctor` (the full report) or `e providers` (its provider table), as
/// text or JSON.
...
async fn run_startup_hooks(
    ...
    args: Vec<String>,
    json: bool,
) -> e::core::extensions::StartupAction {
...
/// Parse the post-hook argv against e's flags and the extensions' own, and
...
async fn parse_options(host: &e::core::extensions::ExtensionHost, args: &[String]) -> Options {
...
/// `e help`, the subcommand form of `e --help`. True when it ran.
async fn help_command(host: &e::core::extensions::ExtensionHost, args: &[String]) -> bool {
...
/// `e auth`: print sign-in status. True when it ran.
async fn auth_command(host: &e::core::extensions::ExtensionHost, options: &Options) -> bool {
...
/// `e -p`: one headless turn, then a clean exit with the turn's status.
...
/// `e update`: replace this release build with the latest one. Every
/// one-shot exit owes extensions their shutdown notification.
async fn update_command(host: &e::core::extensions::ExtensionHost) {
...
/// `e docs [topic]`: print one embedded guide, or the list of them.
async fn docs_command(host: &e::core::extensions::ExtensionHost, args: &[String]) {
...
async fn run_interactive(
    ...
    requests: (
        tokio::sync::mpsc::Sender<e::core::extensions::HostRequest>,
        tokio::sync::mpsc::Receiver<e::core::extensions::HostRequest>,
    ),
) -> std::io::Result<()> {
...
async fn relay_turn(
    ...
    result: &mut e::rpc::TurnAccumulator,
    json: bool,
) -> bool {
