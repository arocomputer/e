...
use ulo::core::providers::{ResponseMeta, ResponsePurpose, Usage};
use ulo::core::session::SessionLog;

fn response(
    model: &ulo::core::providers::catalog::Model,
    at_ms: u64,
    input: u64,
    output: u64,
) -> ResponseMeta {
...
fn usage_folds_responses_by_model_within_the_period() {
    ...
    let all = ulo::core::usage::report(&paths, None);
    ...
    let copied = ulo::core::session::responses_in(b.path()).remove(0);
    forked.append_response(copied).unwrap();
    let with_fork = [paths.clone(), vec![forked.path().to_path_buf()]].concat();
    assert_eq!(ulo::core::usage::report(&with_fork, None).requests, 4);

    let recent = ulo::core::usage::report(&paths, Some(5_000));
    assert_eq!(
        recent.requests, 3,
    ...
    let table = ulo::core::usage::markdown(&recent, "last 7 days");
    ...
    assert!(ulo::core::usage::markdown(
        &ulo::core::usage::report(&paths, Some(u64::MAX)),
        "last 24 hours"
    )
    .contains("No responses recorded"));

    assert_eq!(ulo::core::usage::period_ms("24h"), Some(Some(86_400_000)));
    assert_eq!(ulo::core::usage::period_ms(""), Some(Some(7 * 86_400_000)));
    assert_eq!(ulo::core::usage::period_ms("all"), Some(None));
    assert_eq!(ulo::core::usage::period_ms("fortnight"), None);
}
