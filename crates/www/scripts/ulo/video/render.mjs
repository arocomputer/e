...
if (!chrome)
  throw new Error(
    "Usage: node scripts/ulo/video/render.mjs /path/to/chrome [dev-origin]",
  );
const route = join(root, "src/pages/ulo/demo-export.astro");
try {
  await access(route);
...
try {
  await writeFile(
    route,
    '---\nimport VideoFrame from "../../sites/ulo/components/demo/export/frame";\n---\n<VideoFrame client:load />\n',
  );
  browser = spawn(
    chrome,
    [
  ...
  await send("Page.navigate", { url: `${origin}/ulo/demo-export` });
  let duration;
  ...
  const output = join(root, "public/ulo/demo.mp4");
  execFileSync("ffmpeg", [
    "-hide_banner",
    "-loglevel",
    "error",
  ...
  execFileSync("ffmpeg", [
    ...
    "-frames:v",
    "1",
    join(root, "public/ulo/demo-poster.jpg"),
  ]);
