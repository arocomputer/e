...
Run commands from `crates/www/` with Node.js 22 or newer.
...
Mock contact responses. Verify editable review, retained failed drafts,
unchecked subscription consent, validation, and retry feedback without
sending real messages. The production contact backend remains on Aro through
a service binding.

## Generated files

`dist/`, `.astro/`, `.wrangler/`, `src/generated/`, and
`public/ulo/pagefind/` are ignored build output. The video, poster, social
