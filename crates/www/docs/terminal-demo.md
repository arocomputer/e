...
The homepage plays `public/ulo/demo.mp4` through a native video element. Source
...
| File                                                  | Purpose                                               |
| ----------------------------------------------------- | ----------------------------------------------------- |
| `src/sites/ulo/components/demo/player.tsx`            | Native media clock, visibility, and controls          |
| `src/sites/ulo/styles/demo.css`                       | Video sizing and playback line                        |
| `src/sites/ulo/components/demo/export/recording.json` | Reviewed cells captured from the real ulo TUI         |
| `src/sites/ulo/components/demo/export/timeline.ts`    | Prompt speed and cursor-following camera              |
| `src/sites/ulo/components/demo/export/screen.tsx`     | Captured colors and glyphs                            |
| `src/sites/ulo/components/demo/export/screen.css`     | Terminal geometry for export                          |
| `src/sites/ulo/components/demo/export/frame.tsx`      | Frame renderer driven by the export clock             |
| `scripts/ulo/video/render.mjs`                        | Deterministic screenshots, H.264 encoding, and poster |
| `scripts/ulo/recording/record.py`                     | Isolated provider-backed editing session              |
| `scripts/ulo/recording/export.py`                     | Convert raw PTY evidence to reviewed cells            |
...
```sh
python3 scripts/ulo/recording/record.py
python3 scripts/ulo/recording/export.py /printed/evidence/directory /path/to/ulo
```
...
```sh
node scripts/ulo/video/render.mjs /path/to/chrome http://localhost:3107
```

The renderer creates a temporary `/ulo/demo-export` route and an isolated headless
