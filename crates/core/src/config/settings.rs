...
pub fn get_string(key: &str) -> Option<String> {
    crate::config::store::read_object(&home::settings_path())
        .unwrap_or_default()
        .get(key)
...
pub fn set_string(key: &str, val: &str) -> std::io::Result<()> {
    crate::config::store::update_versioned(&home::settings_path(), 0o644, FORMAT_VERSION, |obj| {
        stamp_format(obj);
...
pub fn theme_names() -> Vec<String> {
    ...
    let mut dirs = vec![(
        home::themes_dir(),
        crate::resources::packages::Filter::default(),
    )];
    dirs.extend(crate::resources::packages::dirs("themes"));
