...
use crate::providers::{catalog, Usage};
...
pub fn report(session_paths: &[std::path::PathBuf], since_ms: Option<u64>) -> Report {
    ...
    for path in session_paths {
        for response in crate::session::responses_in(path) {
...
pub fn report_for(period: &str) -> Option<Report> {
    ...
    let paths: Vec<std::path::PathBuf> = crate::session::list_all()
        .into_iter()
        .map(|info| info.path)
        .collect();
...
pub fn markdown(report: &Report, period_label: &str) -> String {
    use crate::output::{format_cost, format_tokens};
