...
pub async fn install_release_from(source: &Source, base: &str, api: &str) -> Result<(), String> {
    ...
    let wanted = match tag {
        Some(tag) => tag.clone(),
        None => crate::update::latest_tag_from(api)
            .await?
            .ok_or("the repository has no releases")?,
    };
    if crate::update::installed_release_tag(&root).as_deref() == Some(wanted.as_str())
        && root.join("extensions").join(name).is_file()
    {
        return Ok(());
    }
    crate::update::install_release_package(base, &wanted, name, &root).await
}
...
impl Filter {
    ...
    pub fn allows(&self, kind: &str, name: &str) -> bool {
        ...
        let matches = |pattern: &str| {
            crate::tools::glob_regex(pattern).is_ok_and(|re| re.is_match(&candidate))
        };
...
pub async fn use_once(spec: &str) -> Result<PathBuf, String> {
    ...
    let root = match &source {
        ...
        Source::Release {
            ...
            crate::update::install_release_package(&base, &wanted, name, &dir).await?;
            dir
        }
    };
    once_roots()
...
const SCAFFOLD: &str = include_str!("../../guides/extend/examples/scaffold.mjs");
...
// docs/guides/extend/extensions.md, the helper beside this file is optional.
