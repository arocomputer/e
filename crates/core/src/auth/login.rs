...
pub async fn access_token(provider: &str) -> Result<String, String> {
    match auth::load().get(provider).cloned() {
        ...
        Some(Credential::OAuth {
            ...
            let flow =
                crate::providers::registry::find(provider).and_then(|p| p.auth.oauth.clone());
            match flow.as_deref() {
        ...
        None if crate::providers::registry::find(provider).is_some_and(|p| p.auth.none) => {
...
fn xai_credential(
...
) -> Result<crate::auth::Credential, String> {
    ...
    Ok(crate::auth::Credential::OAuth {
        access,
        refresh,
        ...
        crate::config::home::scope(home.clone(), async {
            crate::auth::set(
                ...
                crate::auth::Credential::OAuth {
                    ...
                    expires: crate::auth::now_ms() + 120_000,
