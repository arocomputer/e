...
//! and `session.*` families documented in docs/guides/extend/extensions.md. The two id
...
pub enum Incoming {
    ...
    ToolUpdate {
        id: u64,
        stream: crate::tools::OutputStream,
        chunk: String,
    },
...
mod tests {
    ...
    fn tool_updates_are_typed_and_correlated() {
        ...
        match parsed {
            Incoming::ToolUpdate { id, stream, chunk } => {
                assert_eq!(id, 7);
                assert_eq!(stream, crate::tools::OutputStream::Stderr);
    ...
    fn released_v1_manifest_fixture_remains_readable() {
        let fixture = include_str!(concat!(
            ...
            "/../cli/tests/fixtures/extensions/v1-manifest.json"
        ));
