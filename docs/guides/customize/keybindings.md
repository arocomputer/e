...
`~/.ulo/keybindings.json` overrides the composer's line-editing keys. It follows
...
to ulo's built-in bindings untouched.
...
`-`, as in `ctrl+-` and `ctrl++`. ulo reads modifiers off the front, and
...
Use `"none"` to unbind a built-in chord. ulo swallows the key instead of typing
it as a literal character.

## Which chords reach the keymap

ulo's application-level shortcuts run first. Only chords they do not claim
...
set here. It is `ctrl+t` by default, and you set it in `~/.ulo/layout.json`. See
`docs/guides/customize/layout.md`.

## External editor

ctrl+g opens the draft in an external editor. ulo uses the first of these that
is set:

1. the `editor` setting in `~/.ulo/settings.json`, such as
   `"editor": "code --wait"`
2. `$VISUAL`
3. `$EDITOR`
4. `vi`
...
previous sessions. ulo keeps the newest thousand in `~/.ulo/history.jsonl`,
...
- `Ctrl+O` or `Esc` closes the reader. `Ctrl+C` closes it and keeps ulo's global
...
Set `transcript_hint` in `~/.ulo/settings.json` to override the footer wording.
...
These preferences in `~/.ulo/settings.json` take effect in a new editor:
...
`"trust_scroll_hint"` in `~/.ulo/settings.json`.
