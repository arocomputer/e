...
A skill is a directory with a `SKILL.md` file: `~/.e/skills/<name>/SKILL.md`.
...
e reads skill files on each use. Add or edit a skill and it is live
immediately.

## How the model uses a skill

e advertises a catalog in the system prompt. Each entry holds the skill's
...
A trusted repository can carry its own skills in `.e/skills/`, as
`<repo>/.e/skills/<name>/SKILL.md`, in the same format as above.
