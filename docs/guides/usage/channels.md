---
title: Channels
description: Put ulo in Slack, GitHub, or Linear as a client of ulo rpc.
order: 3
---

# Channels

A channel puts ulo where a team already works, such as a Slack thread, a
Linear issue, or a GitHub pull request.

Each channel is a small program of its own. It spawns `ulo rpc`, maps the
platform's conversations to ulo sessions, and relays events back. ulo does not
...
1. **Spawn `ulo rpc`.** Keep the pipes open and say `hello`. `ask` lines do
...
A channel never parses terminal output and never touches `~/.ulo` itself.
...
`ULO_CWD`. The bot posts tool progress as it happens and the reply when the
...
- Put ulo's home on a volume.
- Record the trust decision once with `ulo trust`.
...
install ulo, and run `ulo -p --json` with the comment as the prompt. It posts
...
If a conversation must continue across comments, use `ulo rpc` with
`save: true` and cache `~/.ulo/sessions` between runs.
