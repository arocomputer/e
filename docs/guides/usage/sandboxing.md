...
e has no built-in permission system. It runs with the permissions of the user
and process that launch it. The isolation has to come from outside e.
...
Local storage still has privacy boundaries. On Unix, e does the following:
...
[compatibility](../extend/compatibility.md) for how e handles older session
files.

## The build guard

`scripts/guard.sh` is a different thing and does not cover this. It audits
*e's own build*, not the permissions of a running session. It checks:
...
A clean `guard.sh` says nothing about what a live `e` process can reach.
...
boundary protects e's own integrity, not the machine's.

## Packages

Packages installed with `e install` widen this exposure. They do not narrow
...
The `tool_call` hook is what e gives you. Extensions can use it to gate
...
**Container the whole session.** Run `e` itself inside Docker or an equivalent.
...
**Use a restricted user or VM.** Run `e` as a low-privilege user, or inside a
VM. Choose this when a container's shared kernel isn't isolation enough for
your threat model.

**Sandbox just `bash` at the OS level.** e's extension protocol lets a tool
...
answer. It is built to give e a real execution boundary directly, instead of
