...
in `services/www/`; its guides and installer come from this repository. Aro's
website and existing email subscriptions remain in `arocomputer/web`.
...
DNS and certificates. Contact is a direct email link to `contact@aro.computer`.
The website has no email backend or service binding. Existing unsubscribe
links redirect to Aro, which issued those emails. The `v2` migration removes
the website's retired contact rate limiter.
