...
- Short conversations use a compact layout by default. In `/settings`, change
  **TUI Mode** from **Inline** to **Fullscreen** to pin the composer to the
  ...
  and persist in `~/.ulo/settings.json` as `"tui_mode": "inline"` or
...
`"tool_label_rows"` in `~/.ulo/settings.json` changes this budget from 1 through 20
...
`"tool_preview_rows"` in `~/.ulo/settings.json` controls the live preview budget.
...
```sh
ULO_PTY_ARTIFACTS=/tmp/ulo-rendering-review cargo test --test pty_stream
python3 scripts/term.py /tmp/ulo-rendering-review/PTY_STREAM_FINISHED.raw 100 30
python3 scripts/term.py /tmp/ulo-rendering-review/PTY_TOOL_FINISHED.raw 100 30
```
...
available. Actual system sleep can suspend both; ulo estimates some sleep gaps
...
After output, including partial tool arguments, ulo retains the partial response
...
Short headlines can be overridden in `~/.ulo/settings.json` with `error_auth`,
...
occurs, with the same scoped ulo home as the failed turn.
