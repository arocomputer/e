...
fn store_path() -> std::path::PathBuf {
    crate::core::config::home::home().join("models-dev.json")
}
...
pub(super) async fn refresh(max_age_ms: u64) {
    ...
    let path = store_path();
    let stored = crate::core::config::store::read_object(&path).unwrap_or_default();
    let fresh = stored
        .get("checked_at")
        .and_then(|v| v.as_u64())
    ...
    let Ok(client) = crate::core::providers::http() else {
        return;
    };
    ...
    if response.status() == reqwest::StatusCode::NOT_MODIFIED {
        let _ = crate::core::config::store::update(&path, 0o644, |obj| {
