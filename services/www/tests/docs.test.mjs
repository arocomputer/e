...
import { renderMarkdown } from "../src/components/docs/markdown.ts";
import {
  build,
  callouts,
  docsRef,
...
test("a guide becomes a page with its sections and subsections", async () => {
  ...
  assert.equal(models.route, "/docs/models");
...
test("links point at routes, or at the repository when they are not pages", async () => {
  ...
  const body = index.docs.find((entry) => entry.slug === "models").sections[0]
    .body;
  assert.match(body, /\]\(\/docs\/sessions\)/);
  assert.match(
    body,
...
test("guides under docs/guides link out of the repository from their real path", async () => {
  ...
  assert.match(
    index.docs[0].lead,
    /\]\(https:\/\/github\.com\/arocomputer\/ulo\/blob\/main\/docs\/contributing\/releases\.md\)/,
  );
...
test("the getting-started guide is the docs root", async () => {
  ...
  assert.equal(home.route, "/docs");
  const body = index.docs.find((entry) => entry.slug === "models").sections[0]
    .body;
  assert.match(body, /\]\(\/docs\)/, "a link to it is not the root");
});
