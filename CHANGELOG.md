...
- Install the Slack channel from npm (`npm install -g @intuitums/e-slack`, or `npx @intuitums/e-slack`). It ships with each release under the same version and tag as the binary it drives.
...
- Run the Slack channel on a server from `channels/slack/Dockerfile`: e comes from the release and the bot from the repository, so the host needs neither Node nor a checkout of e.
