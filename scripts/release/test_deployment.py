                ...
                with self.subTest(channel=channel, outcome=outcome):
                    needs = {
                        'resolve': {'result': 'success', 'outputs': {
                            'channel': channel, 'repository': 'intuitums/e-beta' if channel == 'beta' else 'arocomputer/e', 'sha': 'a' * 40, 'tag': 'v1.2.3', 'version': '1.2.3'}},
                    ...
                    self.assertEqual(status['environment_url'],
                                     (('https://www.npmjs.com/package/@intuitums/e/v/1.2.3' if channel == 'dev' else f'https://github.com/{"intuitums/e-beta" if channel == "beta" else "arocomputer/e"}/releases/tag/v1.2.3') if outcome == 'success' else status['log_url']))
