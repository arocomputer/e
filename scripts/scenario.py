...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='ulo-scenario-') as tmp:
        root = Path(tmp)
        ...
        env = {'PATH': os.environ.get('PATH', ''), 'HOME': tmp, 'ULO_HOME': str(state),
        ...
        try:
            subprocess.run([str(ROOT / 'target/debug/ulo'), '--no-extensions', '--model', 'mock/audit', *([] if args.scenario == 'tools' else ['--no-tools']), prompt], cwd=project, env=env, check=False)
            if args.scenario == 'resume':
                subprocess.run([str(ROOT / 'target/debug/ulo'), '--no-extensions', '--no-tools', '--model', 'mock/audit', '--continue'], cwd=project, env=env, check=False)
        finally:
            server.shutdown()
