...
/// Highlight with caller-owned tokens for keywords, strings, numbers, comments,
/// functions and types. Empty tokens leave that category unstyled.
pub fn highlight_block_with_tokens(
    theme: &Theme,
    lang: &str,
    source: &str,
    tokens: [&str; 6],
    ...
    let [kw, strn, num, com, function, ty] = tokens.map(|token| theme.fg_prefix(token));
...
mod tests {
    ...
    fn theme() -> Theme {
        Theme::from_json(r#"{"vars":{},"colors":{"syntaxKeyword":252,"syntaxString":250,"syntaxNumber":248,"syntaxComment":244}}"#).unwrap()
    }
