...
usage() {
  echo "usage: ./x [dev|scenario|preview|install-dev|hooks|check|fmt|lint|test|crates|docs|guard|packages|channels|container|ui|bench|sbom|release-check] [args...]" >&2
  exit 2
}
